(function () {
  ...
  var library = (window.STARLIGHT_SONGS || []).slice();
  ...
  function renderLibrary() {
    ...
    libraryCount.textContent = library.length + (library.length === 1 ? ' song' : ' songs');

    if (!library.length) {
      var empty = document.createElement('div');
      empty.className = 'empty-note';
      empty.textContent = 'No songs yet — upload one to get started.';
    ...
    library.forEach(function (song) {
      ...
      var explicitTag = song.isExplicit
        ? '<span class="row-explicit" title="Explicit">E</span>'
        : '';
  ...
  function currentIndex() {
    ...
    return library.findIndex(function (s) { return String(s.id) === String(currentSong.id); });
  }

  function playNext() {
    var idx = currentIndex();
    if (idx === -1 || idx + 1 >= library.length) return;
    loadSong(library[idx + 1], true);
  }
  ...
  playBtn.addEventListener('click', function () {
    if (!currentSong) {
      if (library.length) loadSong(library[0], true);
      return;
    }
    if (audio.paused) {
      var p = audio.play();
  ...
  uploadInput.addEventListener('change', function (e) {
    ...
    library.unshift(uploadedSong);
    renderLibrary();
    loadSong(uploadedSong, true);
  ...
  var initialSong = null;
  try {
    var savedId = sessionStorage.getItem('starlight_now_playing_id');
    if (savedId) {
      initialSong = library.find(function (s) { return String(s.id) === String(savedId); });
    }
  } catch (e) {}

  if (!initialSong && library.length) {
    initialSong = library[0];
  }
